...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % Titolo, una riga — serif calda (Libertine)
  \node[anchor=north west, text=white, align=left, font=\bfseries\rmfamily\fontsize{30}{34}\selectfont]
    at (0.7,13.5) {Prenota online};

  % Sottotitolo — ricalibrato verticalmente per non sovrapporsi
  \node[anchor=north west, text=white!88!studioteal, align=left, font=\sffamily\fontsize{13}{16}\selectfont]
    at (0.7,12.1) {Ambulatori di Arceto e Casalgrande};

  % ---- Pillola con l'indirizzo del sito, dentro la fascia teal -----------
  % Ricalibrata verticalmente per evitare sovrapposizioni col sottotitolo
  \node[
  ...
  ] at (\larghezza/2,10.8) {studiomedicobragliavaleria.it};
  ...
  % Voci riordinate: "Aggiungi come app" è passata in seconda posizione (y=7.70)
  \riga{9.30}{Registrati}{Crea il tuo account in un minuto}
  \riga{7.70}{Aggiungi il sito come app}{Un'icona sul telefono, gratis,\\attiva anche le notifiche}
  \riga{6.10}{Prenota una visita}{Scegli ambulatorio, giorno e orario\\dal calendario}
  \riga{4.50}{Richiedi ricette e impegnative}{Medicinali, visite specialistiche,\\esami del sangue}
  \riga{2.90}{Ricevi un avviso}{Una notifica quando la tua\\richiesta è pronta}

  % Icone (linee bianche sopra i quadratini teal disegnati da \riga)

  % Persona (registrati) - 1° posizione
  \begin{scope}[shift={(1.15,9.30)}, white, line width=1.1pt, line cap=round, line join=round]
    \draw (0,0.10) circle (0.135);
    \draw (-0.26,-0.30) arc (180:360:0.26cm and 0.20cm);
  \end{scope}

  % Telefono (aggiungi) - 2° posizione
  \begin{scope}[shift={(1.15,7.70)}, white, line width=1.1pt, line cap=round, line join=round,
                rounded corners=0.07cm]
    \draw (-0.20,-0.36) rectangle (0.20,0.36);
    \draw (-0.08,0.26) -- (0.08,0.26);
  \end{scope}

  % Calendario (prenota) - 3° posizione
  \begin{scope}[shift={(1.15,6.10)}, white, line width=1.1pt, line cap=round, line join=round]
  ...
  % Capsula (medicinale) - 4° posizione
  \begin{scope}[shift={(1.15,4.50)}, rotate=45]
  ...
  % Campanella (notifica) - 5° posizione
  \begin{scope}[shift={(1.15,2.90)}, white, line width=1.1pt, line cap=round, line join=round]
  ...
  ] (cta) at (\larghezza/2,1.35) {Scopri il sito \ $\rightarrow$};

  \node[anchor=north, text=studiogrigio, font=\sffamily\fontsize{10.5}{13}\selectfont]
    at (\larghezza/2,0.65) {studiomedicobragliavaleria.it};
